...
interface AdminPwaInstallProps {
  variant?: 'header' | 'sidebar' | 'login';
  onAction?: () => void;
}

export function AdminPwaInstall({ variant = 'header', onAction }: AdminPwaInstallProps = {}) {
    ...
    <>
      ...
      {variant === 'sidebar' && (
        <button
          ...
          className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl bg-gradient-to-r from-brand-600/90 to-brand-500/90 hover:from-brand-500 hover:to-brand-400 text-white font-bold text-xs shadow-glow-brand transition-all active:scale-[0.98] border border-brand-400/30 group"
        ...
        >
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="text-left min-w-0">
              <div className="font-extrabold text-white text-xs leading-tight truncate">Install Admin App</div>
              <div className="text-[10px] text-brand-100 font-medium opacity-85 truncate">Mobile & Desktop PWA</div>
            </div>
          </div>
          <Sparkles className="w-3.5 h-3.5 text-accent-300 animate-pulse flex-shrink-0 ml-1" />
        </button>
      )}
      ...
      {variant === 'login' && (
        <button
          ...
          className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-brand-400 hover:text-brand-300 text-xs font-bold transition-all border border-slate-700/80 active:scale-95"
          aria-label="Install Admin Console App"
          title="Install Kangra Hub Admin Console as an App"
        >
          <Download className="w-3.5 h-3.5 flex-shrink-0" />
          <span>Install Admin App</span>
          <Sparkles className="w-3 h-3 text-accent-400 ml-0.5" />
        </button>
      )}
      ...
      {variant === 'header' && (
        <button
          onClick={handleOpenModal}
          className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 bg-gradient-to-r from-brand-600 to-brand-500 hover:from-brand-500 hover:to-brand-400 text-white rounded-xl text-xs font-bold transition-all shadow-glow-brand active:scale-95 focus:outline-none focus:ring-2 focus:ring-brand-400 border-none flex-shrink-0"
          aria-label="Install Admin Console App"
          title="Install Kangra Hub Admin Console as an App"
        >
          <Download className="w-3.5 h-3.5 flex-shrink-0" />
