        ...
        {diagLoading || !diagData ? (
          ...
          <div className="space-y-4 text-xs font-mono">
            <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-between">
              <div>
                <strong className="text-sm text-slate-900 block font-sans">{diagData.bank_name}</strong>
                <span className="text-slate-500 text-[11px]">{diagData.file_name}</span>
              </div>
              <Badge variant={diagData.confidence_tier === 'HIGH' ? 'success' : 'warning'} size="sm">
                {diagData.confidence_tier} ({diagData.confidence_score}%)
              </Badge>
            </div>

            <div className="p-3 bg-slate-900 text-slate-200 rounded-xl space-y-2">
              <div className="text-emerald-400 font-bold uppercase text-[10px]">Balance Verification:</div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px]">
                <div>Opening: ₹{diagData.opening_balance || '0.00'}</div>
                <div className="text-emerald-400">Credits: +₹{Number(diagData.total_credit || 0).toFixed(2)}</div>
                <div className="text-rose-400">Debits: -₹{Number(diagData.total_debit || 0).toFixed(2)}</div>
                <div>Closing: ₹{diagData.closing_balance || '0.00'}</div>
              </div>
            </div>

            {diagData.detection_reasons && (
              <div>
                <span className="text-slate-500 uppercase text-[10px] block mb-1">Signatures:</span>
                <div className="flex flex-wrap gap-1">
                  {diagData.detection_reasons.map((r: string, idx: number) => (
                    <span key={idx} className="px-2 py-0.5 bg-slate-100 rounded text-[10px] text-slate-700">
                      ✓ {r}
                    </span>
                  ))}
                </div>
            ...
            <div className="pt-2 flex items-center justify-between">
              ...
              <a
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold text-white bg-brand-600 hover:bg-brand-700 shadow-xs"
              >
                <Download className="w-3.5 h-3.5" />
                Download XML
              </a>
