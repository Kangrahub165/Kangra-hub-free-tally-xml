...
Eliminates cross-row narration mixing (e.g. ATM-CASH, party names leaking to next row).
"""
import logging
...
class AxisBankParser(BaseStatementParser):
    """
    Dedicated parser for Axis Bank statements.
    Extracts 7-column structured tables: Tran Date, Chq No, Particulars, Debit, Credit, Balance, Branch.
    Guarantees strict boundary isolation so narrations never leak across rows.
    """
    ...
    def can_parse(self, doc: ExtractedDocument) -> bool:
        ...
        return "AXIS BANK" in first_page or "UTIB0" in first_page or "AXISBANK.COM" in first_page
    ...
    def parse(self, doc: ExtractedDocument) -> CanonicalStatement:
        ...
        if doc.file_path:
            try:
                ...
                logger.error(f"AxisBankParser failed to extract table: {e}", exc_info=True)

        # Infer opening balance if balance is present
